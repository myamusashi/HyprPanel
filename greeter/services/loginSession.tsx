...
class LoginSession {
    ...
    public profilePic = Variable(`/home/gilang/face.png`);
    ...
    private constructor() {
        this.userName.call('changed', () => {
            this.profilePic.set(this.users[0]);
            this.password.set('');
        });
    }
    ...
    private async initialize(): Promise<void> {
        ...
        this.userName.set(this.users[0]);

        const firstSessionInList = this.sessions[0];
        this.currentSession.set(firstSessionInList as DesktopSessions);
        console.log(this.currentSession.get());
    }
    ...
    public getProfilePicture(user: string): string {
        const defaultAvatar = `/home/gilang/face.png`;
    ...
    public getNextUser(): void {
        const indexOfCurrentUser = this.users.indexOf(this.userName.get());
        const totalUsers = this.users.length;
        const nextUserIndex = (indexOfCurrentUser + 1) % totalUsers;

        this.userName.set(this.users[nextUserIndex]);
    }
    ...
    public getPrevUser(): void {
        ...
        this.userName.set(this.users[prevUserIndex]);
    }
    ...
    async login(): Promise<void> {
        ...
        if (this.currentSession.get().length === 0) {
            return;
        }

        // AstalGreet.login(this.userName.get(), this.password.get(), this.currentSession.get()), (res) => {
        //     try {
        ...
        // Greet.login(this.userName.get(), this.password.get(), "Hyprland", (_, res) => {
        //     try {
